...
class RunControllerTest {
    ...
    @Test
    void return_all_runs() throws Exception {
    ...
    void shouldReturnNotFoundWithInvalidId() throws Exception {
        mockMvc.perform(get("/api/v1/runs/99"))
                .andExpect(status().isNotFound());
    }

    @Test
    void shouldCreateNewRun() throws Exception {
        var run = new Run(null, "test", LocalDateTime.now(), LocalDateTime.now(), 1, Location.INDOOR, 1);
    ...
    void shouldUpdateRun() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/runs/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(run))
                )
    ...
    void shouldDeleteRun() throws Exception {
        when(repository.findById(1)).thenReturn(Optional.of(runs.getFirst()));

        mockMvc.perform(delete("/api/v1/runs/1"))
                .andExpect(status().isNoContent());
    }
}
